...
public:
                                                             ...
                                                             "BG_SUBSTRACTOR_PPLP_STOP")
  {
    ...
    if (!bg_prefix.empty()) {
      has_depth_bg_model = vision_utils::read_rgb_and_depth_image_from_image_file
    ...
    info << "BackgroundSubstractorPPLP: started with '" << get_start_stopic()
         ...
         << "', publish People results on '" << get_ppl_topic()
  ...
  virtual void process_rgb_depth(const cv::Mat3b & rgb,
    ...
    for (unsigned int bbox_idx = 0; bbox_idx < nbboxes; ++bbox_idx) {
      ...
      _user_mask.setTo(0);
      vision_utils::drawListOfPoints(_user_mask, *comp, (uchar) 255);
      if (!_head_finder.find(_user_mask, head2i)) {
        ROS_WARN("Could not find head for blob #%i!", bbox_idx);
        continue;
      }
      vision_utils::copy2(head2i, head2D);
  ...
  virtual void display(const cv::Mat3b & rgb,
    ...
    cv::imshow("BackgroundSubstractorPPLP_background", vision_utils::depth2viz(_background, vision_utils::FULL_RGB_STRETCHED));
    ...
    if (c == 's') {
      ...
      prefix << "BackgroundSubstractorPPLP_" << vision_utils::timestamp();
