...
int test2() {
  ...
  set_node_num_values(bn, A, 3);
  set_node_num_values(bn, S, 2);
  set_node_num_values(bn, J, 2);

  assignment parent_state;
  // F
  set_node_probability(bn, F, 0, parent_state, 1-0.0001);
  set_node_probability(bn, F, 1, parent_state,   0.0001);
  // A
  set_node_probability(bn, A, 0, parent_state,   0.25);
  set_node_probability(bn, A, 1, parent_state,   0.4);
  set_node_probability(bn, A, 2, parent_state,   0.35);
  // S
  set_node_probability(bn, S, 0, parent_state,   0.5);
  set_node_probability(bn, S, 1, parent_state,   0.5);
  // G
  parent_state.add(F, 0);
  set_node_probability(bn, G, 0, parent_state, 1-0.01);
  set_node_probability(bn, G, 1, parent_state,   0.01);
  parent_state[F] = 1;
  set_node_probability(bn, G, 0, parent_state, 1-0.2);
  set_node_probability(bn, G, 1, parent_state,   0.2);
  // J
  parent_state.clear();
  parent_state.add(F, 0);
  parent_state.add(A, 0);
  parent_state.add(S, 0);
  ...
  for (unsigned int a = 0; a <= 2; ++a) {
    ...
    for (unsigned int s = 0; s <= 1; ++s) {
      parent_state[S] = s;
      set_node_probability(bn, J, 1, parent_state,   0.05);
      set_node_probability(bn, J, 0, parent_state,   1-0.05);
    } // end loop s
  } // end loop a
  ...
  parent_state[S] = 0;
  parent_state[A] = 0;
  set_node_probability(bn, J, 1, parent_state,   0.001);
  set_node_probability(bn, J, 0, parent_state,   1-0.001);
  parent_state[A] = 1;
  set_node_probability(bn, J, 1, parent_state,   0.004);
  set_node_probability(bn, J, 0, parent_state,   1-0.004);
  parent_state[A] = 2;
  set_node_probability(bn, J, 1, parent_state,   0.002);
  set_node_probability(bn, J, 0, parent_state,   1-0.002);

  parent_state[S] = 1;
  parent_state[A] = 0;
  set_node_probability(bn, J, 1, parent_state,   0.005);
  set_node_probability(bn, J, 0, parent_state,   1-0.005);
  parent_state[A] = 1;
  set_node_probability(bn, J, 1, parent_state,   0.002);
  set_node_probability(bn, J, 0, parent_state,   1-0.002);
  parent_state[A] = 2;
  ...
  bayesian_network_join_tree solution(bn, join_tree);
  cout << "Using the join tree algorithm:\n";
  cout << "p(F=0) = " << solution.probability(F)(0) << endl;
  cout << "p(F=1) = " << solution.probability(F)(1) << endl;
  cout << "p(G=0) = " << solution.probability(G)(0) << endl;
  cout << "p(G=1) = " << solution.probability(G)(1) << endl;
  cout << "p(A=0) = " << solution.probability(A)(0) << endl;
  cout << "p(A=1) = " << solution.probability(A)(1) << endl;
  cout << "p(A=2) = " << solution.probability(A)(2) << endl;
  cout << "p(S=0) = " << solution.probability(S)(0) << endl;
  cout << "p(S=1) = " << solution.probability(S)(1) << endl;
  cout << "p(J=0) = " << solution.probability(J)(0) << endl;
  cout << "p(J=1) = " << solution.probability(J)(1) << endl;
  cout << "\n\n\n";

  // a concrete case
  set_node_value(bn, A, 2);
  set_node_as_evidence(bn, A);
  set_node_value(bn, S, 1);
  set_node_as_evidence(bn, S);
  set_node_value(bn, J, 1);
  set_node_as_evidence(bn, J);
  set_node_value(bn, G, 0);
  set_node_as_evidence(bn, G);
  bayesian_network_join_tree solution_with_evidence(bn, join_tree);
  cout << "p(F=1 | A=2, S=1, J=1, G=0) = " << solution_with_evidence.probability(F)(1) << endl;
  return 0;
}
